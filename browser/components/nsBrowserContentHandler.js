...
function maybeSetupSpeculativeConnection(uristring) {
  dump(`\n\n=======> Evelyn: maybeSetupSpeculativeConnection: ${uristring} \n\n`);
  ...
  try {
    ...
    dump(`\n\n=======> Evelyn: call speculativeConnect2: ${uristring} \n\n`);
...
function openWindow(parent, url, target, features, args, noExternalArgs) {
  if (noExternalArgs == NO_EXTERNAL_URIS) {
    ...
    if (args) {
      ...
      dump(`\n\n=======> Evelyn: call maybeSetupSpeculativeConnection from default args: ${args} \n\n`);
  ...
  if (stringArgs) {
    ...
    stringArgs.forEach(function(uri) {
      ...
      // create speculative connections for each uri
      dump(`\n\n=======> Evelyn: call maybeSetupSpeculativeConnection from uri: ${uri} \n\n`);
